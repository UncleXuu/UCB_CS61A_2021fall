    ...
    def vend(self):
        ...
        if self.stock <= 0:
            ...
            if self.balance < self.price:
                return f'You must add ${self.price - current_balance} more funds.'
                ...
                self.balance = 0
                self.stock -= 1
                return f'Here is your {self.name} and ${current_balance - self.price} change.'
...
class Coin:
    def __init__(self, year):
        self.year = year
        self.worth_money = self.cents

    def worth(self):
        if (Mint.present_year - self.year) > 50:
            self.worth_money = self.cents + (Mint.present_year - self.year) - 50
            return self.worth_money
        else:
            return self.worth_money
...
def store_digits(n):
    ...
    if n < 10:
        ...
        while x >= 10:
            x = x // 10
            i += 1
        return Link(x, store_digits(n%(pow(10,i))))
...
def two_list(vals, amounts):
    ...
    if len(vals) == 1:
        ...
        y = Link(vals[0], two_list(vals[1:], amounts[1:]))
        while amounts[0] > 1:
            y = Link(vals[0], y)
            amounts[0] -= 1
        return y
