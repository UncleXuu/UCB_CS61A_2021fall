...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r'\b(CS|cs).*[ABCabc]\b', post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'[0-9][0-9]?:[0-6][0-9][apmAPM]*', text)
...
def area_codes(text):
    ...
    []
    """
    # (?: ...) is non-capturing group, and the group you want to output should into a individual group ()
...
def most_common_code(text):
    ...
    '123'
    """
    list = re.findall(r'([\d]{3})(?:\)?[-\s]?[\d]{3}[-\s]?[\d]{4})',text)
    return list[0]
