...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not next_place.is_hive:
            if len(next_place.bees) != 0 and self.min_range <= range <= self.max_range:
        ...
        while behind is not None:
            if behind.ant is not None:
                ...
                if behind.ant.is_container:
                    if behind.ant.ant_contained is not None:
    ...
    def action(self, gamestate):
        ...
        if self.blocked():
            ...
            if self.scare_turn > 0:
                if not departure.is_hive:
                    if self.slow_turn > 0:
                        self.slow_turn -= 1
                        if gamestate.time % 2 == 0:
                            self.move_to(departure)
                            self.scare_turn -= 1
                    else:
                        self.move_to(departure)
                        self.scare_turn -= 1
                elif departure.is_hive:
                    if self.slow_turn > 0:
                        self.slow_turn -= 1
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target is not None and not target.is_scared:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        # referrence: https://github.com/Lijinyu3/UCB-cs61a-fa21/blob/main/projects/ants/ants.py#L440
